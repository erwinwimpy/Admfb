(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function t(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=t(s);fetch(s.href,i)}})();class se{constructor(){this.routes={},this.currentRoute=null,window.addEventListener("hashchange",()=>this._onHashChange())}addRoute(e,t){this.routes[e]=t}navigate(e){window.location.hash=e}getCurrentPath(){return window.location.hash.slice(1)||"/"}_onHashChange(){const e=this.getCurrentPath();this._resolve(e)}_resolve(e){const t=this.routes[e]||this.routes["/"];t&&(this.currentRoute=e,t(e))}start(){const e=this.getCurrentPath();this._resolve(e)}}const L=new se,ie=Object.freeze(Object.defineProperty({__proto__:null,default:L,router:L},Symbol.toStringTag,{value:"Module"})),j="cipta_finansial_data",K={family:null,accounts:[],transactions:[],assets:{emas:{bsi_gram:0,tring_gram:0,price_per_gram:165e4},kpr:{total:0,paid:0,monthly:0,bank:"",remaining_months:0},arisan:[]},settings:{togetherMode:!1,allowanceBudget:15e5,userName:"Erwin",spouseName:"Bunda",geminiApiKey:""},categories:[]};class oe{constructor(){this._listeners=[],this._state=this._load()}_load(){try{const e=localStorage.getItem(j);if(e){const t=JSON.parse(e);return{...K,...t}}}catch(e){console.warn("Failed to load state",e)}return{...K}}_save(){try{localStorage.setItem(j,JSON.stringify(this._state))}catch(e){console.warn("Failed to save state",e)}this._notify()}_notify(){this._listeners.forEach(e=>e(this._state))}subscribe(e){return this._listeners.push(e),()=>{this._listeners=this._listeners.filter(t=>t!==e)}}getState(){return this._state}setFamily(e){this._state.family=e,this._save()}updateSettings(e){this._state.settings={...this._state.settings,...e},this._save()}toggleTogetherMode(){return this._state.settings.togetherMode=!this._state.settings.togetherMode,this._save(),this._state.settings.togetherMode}addAccount(e){const n=this._state.accounts.reduce((s,i)=>Math.max(s,i.id||0),0)+1;return this._state.accounts.push({id:n,...e}),this._save(),n}updateAccount(e,t){const n=this._state.accounts.findIndex(s=>s.id===e);n!==-1&&(this._state.accounts[n]={...this._state.accounts[n],...t},this._save())}deleteAccount(e){this._state.accounts=this._state.accounts.filter(t=>t.id!==e),this._save()}getAccounts(){return this._state.accounts}getAccountById(e){return this._state.accounts.find(t=>t.id===e)}addTransaction(e){const t=crypto.randomUUID?crypto.randomUUID():Date.now().toString(36)+Math.random().toString(36).substr(2),n={id:t,created_at:new Date().toISOString(),...e};return this._state.transactions.unshift(n),e.type==="income"?this._updateAccountBalance(e.account_id,e.amount):e.type==="expense"?this._updateAccountBalance(e.account_id,-e.amount):e.type==="transfer"&&(this._updateAccountBalance(e.account_id,-e.amount),e.to_account_id&&this._updateAccountBalance(e.to_account_id,e.amount)),this._save(),t}_updateAccountBalance(e,t){const n=this._state.accounts.find(s=>s.id===e);n&&(n.balance=(n.balance||0)+t)}deleteTransaction(e){const t=this._state.transactions.find(n=>n.id===e);t&&(t.type==="income"?this._updateAccountBalance(t.account_id,-t.amount):t.type==="expense"?this._updateAccountBalance(t.account_id,t.amount):t.type==="transfer"&&(this._updateAccountBalance(t.account_id,t.amount),t.to_account_id&&this._updateAccountBalance(t.to_account_id,-t.amount)),this._state.transactions=this._state.transactions.filter(n=>n.id!==e),this._save())}getTransactions(e={}){let t=[...this._state.transactions];if(e.type&&(t=t.filter(n=>n.type===e.type)),e.paid_by&&(t=t.filter(n=>n.paid_by===e.paid_by)),e.for_whom&&(t=t.filter(n=>n.for_whom===e.for_whom)),e.account_id&&(t=t.filter(n=>n.account_id===e.account_id)),e.parent_category&&(t=t.filter(n=>n.parent_category===e.parent_category)),e.month&&(t=t.filter(n=>{const s=new Date(n.created_at);return s.getMonth()===e.month&&s.getFullYear()===(e.year||new Date().getFullYear())})),e.search){const n=e.search.toLowerCase();t=t.filter(s=>(s.description||"").toLowerCase().includes(n)||(s.parent_category||"").toLowerCase().includes(n)||(s.sub_category||"").toLowerCase().includes(n))}return t}getTransactionsByMonth(e,t){return this._state.transactions.filter(n=>{const s=new Date(n.created_at);return s.getFullYear()===e&&s.getMonth()===t})}updateAssets(e){this._state.assets={...this._state.assets,...e},this._save()}updateEmas(e){this._state.assets.emas={...this._state.assets.emas,...e},this._save()}updateKPR(e){this._state.assets.kpr={...this._state.assets.kpr,...e},this._save()}addArisan(e){const t=Date.now();return this._state.assets.arisan.push({id:t,...e}),this._save(),t}getAssets(){return this._state.assets}getTotalBalance(){return this._state.accounts.reduce((e,t)=>e+(t.balance||0),0)}getMonthlyExpenses(e,t){return this.getTransactionsByMonth(e,t).filter(n=>n.type==="expense").reduce((n,s)=>n+s.amount,0)}getMonthlyIncome(e,t){return this.getTransactionsByMonth(e,t).filter(n=>n.type==="income").reduce((n,s)=>n+s.amount,0)}getAllowanceSpent(){const e=new Date;return this.getTransactionsByMonth(e.getFullYear(),e.getMonth()).filter(t=>t.type==="expense"&&t.paid_by==="Suami").reduce((t,n)=>t+n.amount,0)}getDanaPusatBalance(){return this._state.accounts.filter(e=>e.owner_name==="Istri"||e.owner_name==="Bersama").reduce((e,t)=>e+(t.balance||0),0)}getNetWorth(){const e=this.getTotalBalance(),t=this._state.assets.emas,n=(t.bsi_gram+t.tring_gram)*t.price_per_gram,s=this._state.assets.kpr.paid;return e+n+s}getCategorySpending(e,t){const n=this.getTransactionsByMonth(e,t).filter(i=>i.type==="expense"),s={};return n.forEach(i=>{const o=i.parent_category||"Lainnya";s[o]=(s[o]||0)+i.amount}),Object.entries(s).map(([i,o])=>({name:i,amount:o})).sort((i,o)=>o.amount-i.amount)}reset(){this._state=JSON.parse(JSON.stringify(K)),localStorage.removeItem(j),this._notify()}}const c=new oe,G=4;function re(){const a=c.getState().accounts;a.length>0&&(a.some(l=>l.bank_name==="Dompet Tunai Suami")||c.addAccount({bank_name:"Dompet Tunai Suami",owner_name:"Suami",balance:5e5,is_allowance_account:!0,css_class:"tunai"}),a.some(l=>l.bank_name==="Dompet Tunai Istri")||c.addAccount({bank_name:"Dompet Tunai Istri",owner_name:"Istri",balance:1e6,is_allowance_account:!0,css_class:"tunai"}));const e=localStorage.getItem("cipta_seed_version");if(e&&parseInt(e)>=G&&c.getState().accounts.length>0)return;c.reset(),c.setFamily({id:"family-001",family_name:"Keluarga Erwin",created_at:new Date().toISOString()}),c.updateSettings({userName:"Pak Erwin",spouseName:"Bunda",allowanceBudget:15e5});const t=c.addAccount({bank_name:"BRI",owner_name:"Suami",balance:85e5,is_allowance_account:!1,css_class:"bri"}),n=c.addAccount({bank_name:"Bank Jago",owner_name:"Bersama",balance:32e5,is_allowance_account:!0,css_class:"jago"}),s=c.addAccount({bank_name:"BSI",owner_name:"Istri",balance:12e6,is_allowance_account:!1,css_class:"bsi"});c.addAccount({bank_name:"Dompet Tunai Suami",owner_name:"Suami",balance:5e5,is_allowance_account:!0,css_class:"tunai"}),c.addAccount({bank_name:"Dompet Tunai Istri",owner_name:"Istri",balance:1e6,is_allowance_account:!0,css_class:"tunai"}),c.updateEmas({bsi_gram:15.5,tring_gram:3.2,price_per_gram:165e4}),c.updateKPR({total:25e7,paid:875e5,monthly:21e5,bank:"BTN",remaining_months:96}),c.addArisan({name:"Arisan Kantor Dinas",monthly_amount:2e5,total_members:15,my_turn:8,current_round:3,is_active:!0}),c.addArisan({name:"Arisan Ibu PKK",monthly_amount:15e4,total_members:20,my_turn:14,current_round:5,is_active:!0});const i=new Date,o=i.getFullYear(),r=i.getMonth(),d=[{account_id:t,amount:72e5,type:"income",description:"Gaji Pokok + Tunjangan April",parent_category:"Gaji & Pendapatan",sub_category:"Gaji Pokok",paid_by:"Suami",for_whom:"Bersama",created_at:new Date(o,r,1,8,0).toISOString()},{account_id:t,amount:35e5,type:"income",description:"TPP Bulan April",parent_category:"Gaji & Pendapatan",sub_category:"TPP",paid_by:"Suami",for_whom:"Bersama",created_at:new Date(o,r,5,10,0).toISOString()},{account_id:t,amount:65e5,type:"transfer",description:"Transfer Dana Pusat ke BSI Istri",parent_category:"Transfer",sub_category:"Pindah Buku",paid_by:"Suami",for_whom:"Bersama",to_account_id:s,created_at:new Date(o,r,1,9,0).toISOString()},{account_id:t,amount:15e5,type:"transfer",description:"Pegangan Suami ke Jago",parent_category:"Transfer",sub_category:"Pindah Buku",paid_by:"Suami",for_whom:"Suami",to_account_id:n,created_at:new Date(o,r,1,9,30).toISOString()},{account_id:n,amount:15e4,type:"expense",description:"Bensin Raize Perjalanan LDM",parent_category:"Transportasi",sub_category:"Bensin LDM",paid_by:"Suami",for_whom:"Suami",created_at:new Date(o,r,2,7,30).toISOString()},{account_id:n,amount:35e3,type:"expense",description:"Tol Barru - Makassar",parent_category:"Transportasi",sub_category:"Tol",paid_by:"Suami",for_whom:"Suami",created_at:new Date(o,r,2,8,0).toISOString()},{account_id:n,amount:25e3,type:"expense",description:"Makan Siang Warung Pak Baso",parent_category:"Makanan & Minuman",sub_category:"Makan Harian",paid_by:"Suami",for_whom:"Suami",created_at:new Date(o,r,3,12,0).toISOString()},{account_id:n,amount:15e3,type:"expense",description:"Kopi Kenangan",parent_category:"Makanan & Minuman",sub_category:"Kopi & Minuman",paid_by:"Suami",for_whom:"Suami",created_at:new Date(o,r,3,15,0).toISOString()},{account_id:n,amount:15e4,type:"expense",description:"Bensin Raize Harian",parent_category:"Transportasi",sub_category:"Bensin Harian",paid_by:"Suami",for_whom:"Suami",created_at:new Date(o,r,5,7,0).toISOString()},{account_id:n,amount:2e5,type:"expense",description:"Arisan Kantor Bulan Ini",parent_category:"Sosial & Ibadah",sub_category:"Arisan",paid_by:"Suami",for_whom:"Suami",created_at:new Date(o,r,6,10,0).toISOString()},{account_id:s,amount:35e4,type:"expense",description:"Belanja Bulanan Indomaret",parent_category:"Makanan & Minuman",sub_category:"Groceries",paid_by:"Istri",for_whom:"Bersama",created_at:new Date(o,r,2,10,0).toISOString()},{account_id:s,amount:5e5,type:"expense",description:"SPP TK Aisyah Bulan April",parent_category:"Pendidikan Anak",sub_category:"SPP",paid_by:"Istri",for_whom:"Anak",created_at:new Date(o,r,3,9,0).toISOString()},{account_id:s,amount:18e4,type:"expense",description:"Listrik Rumah B",parent_category:"Rumah Tangga",sub_category:"Listrik",paid_by:"Istri",for_whom:"Bersama",created_at:new Date(o,r,4,11,0).toISOString()},{account_id:s,amount:21e5,type:"expense",description:"Cicilan KPR BTN April",parent_category:"Cicilan",sub_category:"KPR",paid_by:"Istri",for_whom:"Bersama",created_at:new Date(o,r,5,8,0).toISOString()},{account_id:s,amount:15e4,type:"expense",description:"Arisan Ibu PKK",parent_category:"Sosial & Ibadah",sub_category:"Arisan",paid_by:"Istri",for_whom:"Istri",created_at:new Date(o,r,6,14,0).toISOString()},{account_id:s,amount:85e3,type:"expense",description:"Vitamin & Susu Anak",parent_category:"Kesehatan",sub_category:"Vitamin",paid_by:"Istri",for_whom:"Anak",created_at:new Date(o,r,7,10,0).toISOString()},{account_id:n,amount:25e4,type:"expense",description:"Makan Keluarga di Restoran",parent_category:"Hiburan",sub_category:"Quality Time",paid_by:"Suami",for_whom:"Bersama",is_together:!0,created_at:new Date(o,r,7,19,0).toISOString()},{account_id:n,amount:45e3,type:"expense",description:"Es Krim untuk Anak",parent_category:"Makanan & Minuman",sub_category:"Cemilan",paid_by:"Suami",for_whom:"Anak",is_together:!0,created_at:new Date(o,r,7,20,0).toISOString()},{account_id:s,amount:825e3,type:"expense",description:"Beli Emas BSI 0.5 gram",parent_category:"Investasi",sub_category:"Emas BSI",paid_by:"Istri",for_whom:"Bersama",created_at:new Date(o,r,8,9,0).toISOString()}],u=c.getState();d.forEach(l=>{const m=crypto.randomUUID?crypto.randomUUID():Date.now().toString(36)+Math.random().toString(36).substr(2);u.transactions.push({id:m,...l})}),u.transactions.sort((l,m)=>new Date(m.created_at)-new Date(l.created_at)),localStorage.setItem("cipta_finansial_data",JSON.stringify(u)),localStorage.setItem("cipta_seed_version",G.toString())}const ce="modulepreload",de=function(a){return"/Admfb/"+a},J={},D=function(e,t,n){let s=Promise.resolve();if(t&&t.length>0){let o=function(u){return Promise.all(u.map(l=>Promise.resolve(l).then(m=>({status:"fulfilled",value:m}),m=>({status:"rejected",reason:m}))))};document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),d=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));s=o(t.map(u=>{if(u=de(u),u in J)return;J[u]=!0;const l=u.endsWith(".css"),m=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${u}"]${m}`))return;const y=document.createElement("link");if(y.rel=l?"stylesheet":ce,l||(y.as="script"),y.crossOrigin="",y.href=u,d&&y.setAttribute("nonce",d),document.head.appendChild(y),l)return new Promise((_,g)=>{y.addEventListener("load",_),y.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${u}`)))})}))}function i(o){const r=new Event("vite:preloadError",{cancelable:!0});if(r.payload=o,window.dispatchEvent(r),!r.defaultPrevented)throw o}return s.then(o=>{for(const r of o||[])r.status==="rejected"&&i(r.reason);return e().catch(i)})};function le(){const a=c.getState(),e=a.settings.togetherMode;return`
  ...
  `}function ue(){const a=document.getElementById("together-toggle");a&&a.addEventListener("click",()=>{const t=c.toggleTogetherMode();a.classList.toggle("active",t);const n=a.querySelector(".together-toggle-icon"),s=a.querySelector(".together-toggle-label");n.textContent=t?"favorite":"favorite_border",s.textContent=t?"Together!":"Together",D(async()=>{const{showToast:i}=await Promise.resolve().then(()=>H);return{showToast:i}},void 0).then(({showToast:i})=>{i(t?"💕 Together Mode Aktif!":"Together Mode Nonaktif","info")})});const e=document.getElementById("profile-avatar");e&&e.addEventListener("click",()=>{const n=c.getState().settings.userName||"Erwin",s=n==="Erwin"?"Bunda":"Erwin";confirm(`Ganti sesi dari ${n} ke ${s}?`)&&(c.updateSettings({userName:s}),window.dispatchEvent(new Event("data-updated")),D(async()=>{const{showToast:i}=await Promise.resolve().then(()=>H);return{showToast:i}},void 0).then(({showToast:i})=>{i(`Berhasil login sebagai ${s}`,"success")}))})}const pe=[{path:"/",icon:"dashboard",label:"Beranda"},{path:"/transactions",icon:"receipt_long",label:"Transaksi"},{path:"/accounts",icon:"account_balance_wallet",label:"Rekening"},{path:"/assets",icon:"diamond",label:"Aset"},{path:"/insights",icon:"auto_awesome",label:"AI Insight"}];function me(){const a=L.getCurrentPath();return`
  ...
  `}function _e(){var t;const a=document.getElementById("tx-modal-backdrop");document.getElementById("tx-modal-sheet");const e=document.getElementById("tx-form");window.addEventListener("open-transaction-modal",n=>{var s;ke(((s=n.detail)==null?void 0:s.type)||"expense")}),a==null||a.addEventListener("click",q),document.querySelectorAll("#tx-type-chips .chip").forEach(n=>{n.addEventListener("click",()=>{document.querySelectorAll("#tx-type-chips .chip").forEach(r=>r.classList.remove("selected")),n.classList.add("selected");const s=n.dataset.type,i=document.getElementById("tx-to-account-group"),o=document.getElementById("tx-category-row");i&&(i.style.display=s==="transfer"?"":"none"),o&&(o.style.display=s==="transfer"?"none":"")})}),document.querySelectorAll("#tx-paid-by-chips .chip").forEach(n=>{n.addEventListener("click",()=>{document.querySelectorAll("#tx-paid-by-chips .chip").forEach(s=>s.classList.remove("selected")),n.classList.add("selected")})}),document.querySelectorAll("#tx-for-whom-chips .chip").forEach(n=>{n.addEventListener("click",()=>{document.querySelectorAll("#tx-for-whom-chips .chip").forEach(s=>s.classList.remove("selected")),n.classList.add("selected")})}),(t=document.getElementById("tx-category"))==null||t.addEventListener("change",n=>{const s=he(n.target.value),i=document.getElementById("tx-subcategory");i&&(i.innerHTML='<option value="">Pilih Sub-Kategori</option>'+s.map(o=>`<option value="${o}">${o}</option>`).join(""))}),e==null||e.addEventListener("submit",n=>{var x,w,S,b,f,k,A,h,I,B;n.preventDefault();const s=((x=document.querySelector("#tx-type-chips .chip.selected"))==null?void 0:x.dataset.type)||"expense",i=parseFloat(((w=document.getElementById("tx-amount"))==null?void 0:w.value)||0),o=((S=document.getElementById("tx-description"))==null?void 0:S.value)||"",r=parseInt((b=document.getElementById("tx-account"))==null?void 0:b.value),d=s==="transfer"?parseInt((f=document.getElementById("tx-to-account"))==null?void 0:f.value):null,u=((k=document.getElementById("tx-category"))==null?void 0:k.value)||"",l=((A=document.getElementById("tx-subcategory"))==null?void 0:A.value)||"",m=((h=document.querySelector("#tx-paid-by-chips .chip.selected"))==null?void 0:h.dataset.value)||"Suami",y=((I=document.querySelector("#tx-for-whom-chips .chip.selected"))==null?void 0:I.dataset.value)||"Bersama",_=(B=document.getElementById("tx-date"))==null?void 0:B.value,g=_?new Date(_).toISOString():new Date().toISOString(),v=c.getState();if(!i||i<=0){E("Masukkan nominal yang valid","error");return}c.addTransaction({account_id:r,to_account_id:d,amount:i,type:s,description:o,parent_category:s==="transfer"?"Transfer":u,sub_category:s==="transfer"?"Pindah Buku":l,paid_by:m,for_whom:y,is_together:v.settings.togetherMode,created_at:g}),E("✅ Transaksi berhasil disimpan!"),q(),window.dispatchEvent(new CustomEvent("data-updated"))})}function ke(a="expense"){var d;const e=document.getElementById("tx-modal-backdrop"),t=document.getElementById("tx-modal-sheet"),n=document.getElementById("tx-modal-title");(d=document.getElementById("tx-form"))==null||d.reset(),document.getElementById("tx-date").value=new Date().toISOString().slice(0,16),document.querySelectorAll("#tx-type-chips .chip").forEach(u=>{u.classList.toggle("selected",u.dataset.type===a)});const s=document.getElementById("tx-to-account-group"),i=document.getElementById("tx-category-row");s&&(s.style.display=a==="transfer"?"":"none"),i&&(i.style.display=a==="transfer"?"none":""),c.getState().settings.togetherMode&&(document.querySelectorAll("#tx-paid-by-chips .chip").forEach(u=>{u.classList.toggle("selected",u.dataset.value==="Istri")}),document.querySelectorAll("#tx-for-whom-chips .chip").forEach(u=>{u.classList.toggle("selected",u.dataset.value==="Bersama")}));const r={expense:"Tambah Pengeluaran",income:"Tambah Pemasukan",transfer:"Transfer Antar Rekening"};n&&(n.textContent=r[a]||"Tambah Transaksi"),e==null||e.classList.add("open"),t==null||t.classList.add("open")}function q(){var a,e;(a=document.getElementById("tx-modal-backdrop"))==null||a.classList.remove("open"),(e=document.getElementById("tx-modal-sheet"))==null||e.classList.remove("open")}function we(){return`
  ...
  `}let T=[];function xe(){var _,g;const a=document.getElementById("scan-modal-backdrop"),e=document.getElementById("scan-modal-sheet"),t=document.getElementById("scan-upload-area"),n=document.getElementById("scan-file-input"),s=document.getElementById("tab-text"),i=document.getElementById("tab-scan"),o=document.getElementById("panel-text"),r=document.getElementById("panel-scan"),d=document.getElementById("ai-key-config"),u=document.getElementById("ai-main-app"),l=document.getElementById("btn-save-key"),m=document.getElementById("ai-key-input");function y(){c.getState().settings.geminiApiKey?(d.style.display="none",u.style.display="block"):(d.style.display="block",u.style.display="none")}l==null||l.addEventListener("click",()=>{const v=m.value.trim();if(!v){E("API Key tidak boleh kosong","error");return}c.updateSettings({geminiApiKey:v}),E("API Key Berhasil Disimpan"),y()}),s==null||s.addEventListener("click",()=>{o.style.display="block",r.style.display="none",s.style.background="var(--primary-container)",s.style.color="var(--primary)",s.style.borderColor="var(--primary)",i.style.background="transparent",i.style.color="var(--on-surface-variant)",i.style.borderColor="transparent"}),i==null||i.addEventListener("click",()=>{o.style.display="none",r.style.display="block",i.style.background="var(--primary-container)",i.style.color="var(--primary)",i.style.borderColor="var(--primary)",s.style.background="transparent",s.style.color="var(--on-surface-variant)",s.style.borderColor="transparent"}),window.addEventListener("open-scan-modal",()=>{a==null||a.classList.add("open"),e==null||e.classList.add("open"),y(),U()}),a==null||a.addEventListener("click",()=>{a==null||a.classList.remove("open"),e==null||e.classList.remove("open")}),(_=document.getElementById("btn-analyze-text"))==null||_.addEventListener("click",async()=>{var x,w;const v=(w=(x=document.getElementById("ai-text-input"))==null?void 0:x.value)==null?void 0:w.trim();if(!v){E("Silakan ceritakan pengeluaran Anda dulu","error");return}Y();try{await W(v,null,null)}catch(S){V(S)}}),t==null||t.addEventListener("click",()=>n==null?void 0:n.click()),n==null||n.addEventListener("change",async v=>{var S;const x=(S=v.target.files)==null?void 0:S[0];if(!x)return;Y();const w=new FileReader;w.onload=async b=>{let f=b.target.result;try{f=await Se(f)}catch(k){console.warn("Compression failed, using original",k)}document.getElementById("scan-preview-img").src=f,document.getElementById("scan-preview").style.display="block";try{await W(null,f.split(",")[1],"image/jpeg")}catch(k){V(k)}},w.readAsDataURL(x)}),(g=document.getElementById("scan-save-btn"))==null||g.addEventListener("click",()=>{if(!T||T.length===0)return;const v=c.getAccounts(),w=c.getState().settings.togetherMode;let S=0;T.forEach(b=>{var A;let f=(A=v.find(h=>h.bank_name.toLowerCase().includes("tunai")))==null?void 0:A.id;if(b.account_guess){const h=b.account_guess.toLowerCase(),I=v.find(B=>B.bank_name.toLowerCase().includes(h));I&&(f=I.id)}!f&&v.length>0&&(f=v[0].id);let k=null;if(b.type==="transfer"&&b.to_account_guess){const h=b.to_account_guess.toLowerCase(),I=v.find(B=>B.bank_name.toLowerCase().includes(h));I&&(k=I.id)}c.addTransaction({account_id:f,to_account_id:k,amount:b.amount||0,type:b.type||"expense",description:b.description||"Transaksi AI",parent_category:b.category||"Lainnya",sub_category:b.sub_category||"",paid_by:w?"Istri":"Suami",for_whom:w?"Bersama":"Suami",is_together:w,created_at:b.date?new Date(b.date).toISOString():new Date().toISOString()}),S++}),E(`✅ ${S} transaksi dari AI berhasil disimpan!`),a==null||a.classList.remove("open"),e==null||e.classList.remove("open"),window.dispatchEvent(new CustomEvent("data-updated")),U()})}function U(){document.getElementById("scan-preview").style.display="none",document.getElementById("scan-result").style.display="none",document.getElementById("scan-loading").style.display="none",document.getElementById("scan-parsed-data").style.display="none",document.getElementById("scan-save-btn").style.display="none",document.getElementById("ai-text-input").value="",document.getElementById("scan-file-input").value="",T=[]}function Y(){document.getElementById("scan-loading").style.display="block",document.getElementById("scan-result").style.display="none";const a=document.getElementById("ai-key-config");a&&(a.style.display="none");const e=document.getElementById("ai-main-app");e&&(e.style.display="block")}function V(a){console.error("AI Error:",a),document.getElementById("scan-loading").style.display="none",document.getElementById("scan-result").style.display="block";let e="❌ Maaf, Gagal memproses AI. Silakan coba deksripsi yang lebih jelas.";a.message&&a.message.includes("403")&&(e="❌ API Key tidak valid atau telah diblokir limit. Hapus dan buat key yang baru.",c.updateSettings({geminiApiKey:""}),setTimeout(()=>{document.getElementById("ai-key-config").style.display="block",document.getElementById("ai-main-app").style.display="none",document.getElementById("scan-result").style.display="none"},3e3)),document.getElementById("scan-ai-text").innerHTML=e,document.getElementById("scan-parsed-data").style.display="none",document.getElementById("scan-save-btn").style.display="none"}async function W(a,e,t){var g,v,x,w,S,b;const s=c.getState().settings.geminiApiKey;if(!s)throw new Error("API Key Missing");const i=`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${s}`,o=z.map(f=>f.name).join(", "),r=c.getAccounts();let l=[{text:`Anda adalah Asisten Keuangan Keluarga 'Adam Family'. Tugas Anda mengekstrak transaksi dari cerita informal atau gambar struk.

KONTEKS REKENING YANG TERSEDIA:
${r.map(f=>`- ID ${f.id}: ${f.bank_name} (${f.owner_name})`).join(`
`)}
...
INPUT: "${a||"Analisis gambar struk lampiran"}"`}];e&&l.push({inlineData:{mimeType:t||"image/jpeg",data:e}});const m=await fetch(i,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:l}],generationConfig:{temperature:.1,maxOutputTokens:2048}})});if(!m.ok){const k=((g=(await m.json().catch(()=>({}))).error)==null?void 0:g.message)||m.statusText;throw m.status===403?new Error("403 Forbidden: API Key Problem"):m.status===404?new Error("404 Not Found: Model name error"):m.status===503?new Error("503 Service Busy: Server Google penuh, coba lagi sesaat lagi."):new Error(`HTTP Error ${m.status}: ${k}`)}const _=((b=(S=(w=(x=(v=(await m.json()).candidates)==null?void 0:v[0])==null?void 0:x.content)==null?void 0:w.parts)==null?void 0:S[0])==null?void 0:b.text)||"";document.getElementById("scan-loading").style.display="none",document.getElementById("scan-result").style.display="block";try{const f=_.match(/\[[\s\S]*\]/)||_.match(/\{[\s\S]*\}/);if(!f)throw new Error("AI memberikan respon teks saja, bukan data transaksi.");let k=JSON.parse(f[0]);if(Array.isArray(k)||(k=[k]),T=k,T.length===0)throw new Error("Data transaksi kosong.");document.getElementById("scan-ai-text").innerHTML=`
      <strong>✨ AI Selesai Menganalisis!</strong><br/>
      Saya menemukan ${T.length} transaksi dari cerita Anda.
    `;const A=document.getElementById("scan-parsed-data");A.style.display="block",A.innerHTML=T.map(h=>{const I=r.find(C=>C.id==h.account_id)||{bank_name:"Unknown"},B=h.to_account_id?r.find(C=>C.id==h.to_account_id):null;return`
        <div style="background: var(--surface-container); border-radius: var(--radius-md); padding: 12px; margin-top: 12px; border: 1px solid var(--outline-variant);">
          <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
            <span style="font-weight: 600; color: var(--on-surface-variant);">${h.type==="income"?"💰 Pemasukan":h.type==="transfer"?"🔁 Transfer":"💸 Pengeluaran"}</span>
            <span style="font-weight: 800; color: ${h.type==="income"?"var(--success)":h.type==="transfer"?"var(--primary)":"var(--error)"};">${p(h.amount||0)}</span>
          </div>
          <div style="font-size: 13px; color: var(--on-surface-variant);">
            <p>📝 <strong>${h.description||"-"}</strong></p>
            <p>📂 ${h.category||"-"} ${h.sub_category?"→ "+h.sub_category:""}</p>
            <p>💳 Rekening: <strong>${I.bank_name} ${B?"➡️ "+B.bank_name:""}</strong></p>
          </div>
        </div>
      `}).join(""),document.getElementById("scan-save-btn").style.display="flex"}catch(f){throw console.error(f,_),new Error("Gagal membaca data: "+f.message)}}function Se(a,e=1200){return new Promise((t,n)=>{const s=new Image;s.src=a,s.onload=()=>{const i=document.createElement("canvas");let o=s.width,r=s.height;o>e&&(r=Math.round(r*e/o),o=e),i.width=o,i.height=r,i.getContext("2d").drawImage(s,0,0,o,r),t(i.toDataURL("image/jpeg",.8))},s.onerror=n})}function Ie(){const a=c.getState(),e=new Date,t=a.settings.allowanceBudget||15e5,n=c.getAllowanceSpent(),s=Math.max(0,t-n),i=P(n,t),o=c.getDanaPusatBalance(),r=c.getMonthlyExpenses(e.getFullYear(),e.getMonth()),d=c.getMonthlyIncome(e.getFullYear(),e.getMonth());return`
  ...
  `}function Ee(a){const e=a.toLowerCase();return e.includes("bri")?"bri":e.includes("jago")?"jago":e.includes("bsi")?"bsi":"default"}function Be(a){const e=a.toLowerCase();return e.includes("bri")?"BRI":e.includes("jago")?"JGO":e.includes("bsi")?"BSI":a.slice(0,3).toUpperCase()}function Ae(){return`
    ...
    <div class="bank-slider" id="bank-slider">
      ${c.getAccounts().map(e=>{const t=Ee(e.bank_name);return`
  ...
  `}function $e(){const a=document.getElementById("btn-add-account");a&&a.addEventListener("click",()=>{window.dispatchEvent(new CustomEvent("open-account-modal"))});const e=document.getElementById("btn-manage-accounts");e&&e.addEventListener("click",()=>{D(async()=>{const{default:t}=await Promise.resolve().then(()=>ie);return{default:t}},void 0).then(({default:t})=>t.navigate("/accounts"))})}function Te(){const a=c.getState(),e=new Date,t=c.getTransactionsByMonth(e.getFullYear(),e.getMonth()),n=a.assets.kpr,s=P(n.paid,n.total),i=6e5,o=t.filter(m=>m.type==="expense"&&m.parent_category==="Transportasi").reduce((m,y)=>m+y.amount,0),r=P(o,i),d=8e5,u=t.filter(m=>m.type==="expense"&&m.for_whom==="Anak").reduce((m,y)=>m+y.amount,0),l=P(u,d);return`
    <div class="section-header">
  ...
  `}function N(a=10,e={}){let t=c.getTransactions(e);const n=t.length;if(a&&(t=t.slice(0,a)),t.length===0)return`
      <div class="empty-state">
        <span class="material-icons-round">receipt_long</span>
        <h3>Belum Ada Transaksi</h3>
        <p>Mulai catat pengeluaran dan pemasukan Anda</p>
      </div>
    `;const s=ee(t);let i="";return Object.entries(s).forEach(([o,r])=>{i+=`<div class="transaction-group-header">${o}</div>`,r.forEach(d=>{const u=fe(d.parent_category),l=d.type,m=d.type==="expense"?"-":d.type==="income"?"+":"↔",y=c.getAccountById(d.account_id),_=y?y.bank_name:"";i+=`
    ...
    `),i}function F(){document.querySelectorAll(".transaction-item").forEach(a=>{a.addEventListener("click",()=>{const e=a.dataset.txId;window.dispatchEvent(new CustomEvent("view-transaction",{detail:{id:e}}))})})}function Pe(){const a=c.getState();return`
  ...
  `}function Le(){$e(),F()}function Me(){const a=new Date().getHours();return a<11?"Selamat Pagi":a<15?"Selamat Siang":a<18?"Selamat Sore":"Selamat Malam"}let $={};function De(){const a=new Date,e=c.getMonthlyExpenses(a.getFullYear(),a.getMonth()),t=c.getMonthlyIncome(a.getFullYear(),a.getMonth()),n=t-e;return`
  ...
  `}function ze(){F(),document.querySelectorAll("#tx-filters .chip").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll("#tx-filters .chip").forEach(n=>n.classList.remove("selected")),e.classList.add("selected");const t=e.dataset.filter;t==="all"?delete $.type:$.type=t,R()})}),document.querySelectorAll("#tx-paid-filter .chip").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll("#tx-paid-filter .chip").forEach(n=>n.classList.remove("selected")),e.classList.add("selected");const t=e.dataset.paid;t==="all"?delete $.paid_by:$.paid_by=t,R()})});const a=document.getElementById("tx-search");a&&a.addEventListener("input",te(e=>{const t=e.target.value.trim();t?$.search=t:delete $.search,R()},300)),Ce()}function R(){const a=document.getElementById("filtered-transactions");a&&(a.innerHTML=N(null,$),F())}async function Ce(){const a=document.getElementById("category-chart");if(a)try{const e=await D(()=>import("./auto-eE5P6S0m.js"),[]),t=e.default||e.Chart,n=new Date,s=c.getCategorySpending(n.getFullYear(),n.getMonth());if(s.length===0){a.parentElement.innerHTML=`
  ...
  `}function Ke(a){const e=ae(a.bank_name);return`
  ...
  `}function Oe(){var t;const a=document.getElementById("btn-add-new-account"),e=document.getElementById("acc-modal-backdrop");document.getElementById("acc-modal-sheet"),a==null||a.addEventListener("click",()=>Q()),window.addEventListener("open-account-modal",()=>Q()),e==null||e.addEventListener("click",()=>Z()),document.querySelectorAll("#acc-owner-chips .chip").forEach(n=>{n.addEventListener("click",()=>{document.querySelectorAll("#acc-owner-chips .chip").forEach(s=>s.classList.remove("selected")),n.classList.add("selected")})}),(t=document.getElementById("acc-form"))==null||t.addEventListener("submit",n=>{var d,u,l,m,y;n.preventDefault();const s=(u=(d=document.getElementById("acc-bank-name"))==null?void 0:d.value)==null?void 0:u.trim(),i=((l=document.querySelector("#acc-owner-chips .chip.selected"))==null?void 0:l.dataset.value)||"Suami",o=parseFloat(((m=document.getElementById("acc-balance"))==null?void 0:m.value)||0),r=((y=document.getElementById("acc-is-allowance"))==null?void 0:y.checked)||!1;if(!s){E("Masukkan nama bank","error");return}c.addAccount({bank_name:s,owner_name:i,balance:o,is_allowance_account:r,css_class:ae(s).replace("bank-card-icon ","")}),E("✅ Rekening berhasil ditambahkan!"),Z(),window.dispatchEvent(new CustomEvent("data-updated"))}),document.querySelectorAll(".btn-delete-account").forEach(n=>{n.addEventListener("click",s=>{s.stopPropagation();const i=parseInt(n.dataset.accId);confirm("Hapus rekening ini?")&&(c.deleteAccount(i),E("Rekening dihapus"),window.dispatchEvent(new CustomEvent("data-updated")))})})}function Q(){var a,e,t;(a=document.getElementById("acc-form"))==null||a.reset(),(e=document.getElementById("acc-modal-backdrop"))==null||e.classList.add("open"),(t=document.getElementById("acc-modal-sheet"))==null||t.classList.add("open")}function Z(){var a,e;(a=document.getElementById("acc-modal-backdrop"))==null||a.classList.remove("open"),(e=document.getElementById("acc-modal-sheet"))==null||e.classList.remove("open")}function ae(a){const e=a.toLowerCase();return e.includes("bri")?"bri":e.includes("jago")?"jago":e.includes("bsi")?"bsi":e.includes("tunai")||e.includes("cash")?"tunai":"default"}function Ne(a){const e=a.toLowerCase();return e.includes("bri")?"BRI":e.includes("jago")?"JGO":e.includes("bsi")?"BSI":e.includes("tunai")||e.includes("cash")?"CSH":a.slice(0,3).toUpperCase()}function Fe(){const a=c.getAssets(),e=c.getNetWorth(),t=c.getTotalBalance(),n=a.emas,s=n.bsi_gram+n.tring_gram,i=s*n.price_per_gram,o=a.kpr,r=P(o.paid,o.total),d=o.total-o.paid,u=a.arisan||[];return`
          ...
          <div style="background: var(--surface-container); border-radius: var(--radius-md); padding: 12px; margin-bottom: 8px;">
            ...
            <div style="margin-top: 8px;">
              <div class="progress-bar" style="height: 4px;">
                <div class="progress-bar-fill blue" style="width: ${P(l.current_round,l.total_members)}%;"></div>
              </div>
            </div>
          </div>
        `).join("")}
  ...
  `}function Ge(){}function Je(){c.getState();const a=new Date,e=He();return`
  ...
  `}function He(){const a=new Date,e=c.getState(),n=c.getTransactionsByMonth(a.getFullYear(),a.getMonth()).filter(g=>g.type==="expense"),s=n.reduce((g,v)=>g+v.amount,0),i=e.settings.allowanceBudget||15e5,o=c.getAllowanceSpent(),r=[],d=Math.round(o/i*100);d>=80?r.push({title:"Budget Alert",icon:"warning",color:"#e53935",message:`Pegangan ${e.settings.userName} sudah mencapai ${d}% budget. Sisa ${p(i-o)} untuk bulan ini.`,detail:`Budget: ${p(i)} | Terpakai: ${p(o)}`}):d>=50&&r.push({title:"Pemantauan Budget",icon:"info",color:"#fb8c00",message:`${e.settings.userName}, pegangan sudah terpakai ${d}%. Masih ada ${p(i-o)} untuk sisa bulan ini.`,detail:null});const u=n.filter(g=>g.parent_category==="Transportasi").reduce((g,v)=>g+v.amount,0);u>0&&r.push({title:"Transportasi",icon:"directions_car",color:"#9a6a1a",message:`Total pengeluaran transportasi bulan ini: ${p(u)}. ${u>5e5?"Cukup tinggi, pertimbangkan efisiensi perjalanan LDM.":"Masih terkendali, pertahankan!"}`,detail:null});const l=5e5,m=n.filter(g=>g.sub_category==="Groceries"||g.parent_category==="Makanan & Minuman").reduce((g,v)=>g+v.amount,0);m<l&&m>0&&r.push({title:"Apresiasi! 🎉",icon:"celebration",color:"#43a047",message:`${e.settings.spouseName}, bulan ini pengeluaran makanan & groceries ${p(m,!0)}. Hemat ${p(l-m,!0)} yang bisa dialokasikan ke investasi emas!`,detail:null});const y=n.filter(g=>g.is_together);if(y.length>0){const g=y.reduce((v,x)=>v+x.amount,0);r.push({title:"Quality Time 💕",icon:"favorite",color:"#e91e63",message:`Bulan ini ada ${y.length} transaksi saat Together Mode aktif, total ${p(g)}. Waktu berkualitas bersama keluarga itu priceless!`,detail:null})}const _=n.filter(g=>g.for_whom==="Anak").reduce((g,v)=>g+v.amount,0);return _>0&&r.push({title:"Keperluan Anak",icon:"child_care",color:"#1565c0",message:`Total pengeluaran untuk anak bulan ini: ${p(_)}. Investasi terbaik adalah pendidikan anak.`,detail:null}),r.push({title:"Ringkasan Bulanan",icon:"summarize",color:"#30609d",message:`Total pengeluaran ${M()}: ${p(s)} dari ${n.length} transaksi. Saya terus memantau dan akan memberikan insight yang lebih akurat seiring bertambahnya data.`,detail:null}),r}function qe(){var n;const a=new Date,e=c.getCategorySpending(a.getFullYear(),a.getMonth());if(e.length===0)return'<div style="text-align: center; padding: 20px; color: var(--outline);">Belum ada data</div>';const t=((n=e[0])==null?void 0:n.amount)||1;return`
    <div class="card">
  ...
  `}function Ue(){}const Ye=document.getElementById("app"),O={"/":{render:Pe,init:Le},"/transactions":{render:De,init:ze},"/accounts":{render:je,init:Oe},"/assets":{render:Fe,init:Ge},"/insights":{render:Je,init:Ue}};function ne(a){const e=O[a]||O["/"];Ye.innerHTML=`
    ${le()}
    ${me()}
    ${e.render()}
    ${ve()}
    ${be()}
    ${we()}
  `,ue(),ye(),_e(),xe(),e.init(),ge(a),window.scrollTo({top:0,behavior:"instant"})}Object.keys(O).forEach(a=>{L.addRoute(a,ne)});window.addEventListener("data-updated",()=>{const a=L.getCurrentPath();ne(a)});function Ve(){re(),L.start()}document.addEventListener("DOMContentLoaded",Ve);
