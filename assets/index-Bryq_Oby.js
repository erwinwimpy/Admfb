(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function e(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=e(s);fetch(s.href,i)}})();class X{constructor(){this.routes={},this.currentRoute=null,window.addEventListener("hashchange",()=>this._onHashChange())}addRoute(t,e){this.routes[t]=e}navigate(t){window.location.hash=t}getCurrentPath(){return window.location.hash.slice(1)||"/"}_onHashChange(){const t=this.getCurrentPath();this._resolve(t)}_resolve(t){const e=this.routes[t]||this.routes["/"];e&&(this.currentRoute=t,e(t))}start(){const t=this.getCurrentPath();this._resolve(t)}}const x=new X,tt=Object.freeze(Object.defineProperty({__proto__:null,default:x,router:x},Symbol.toStringTag,{value:"Module"})),$="cipta_finansial_data",I={family:null,accounts:[],transactions:[],assets:{emas:{bsi_gram:0,tring_gram:0,price_per_gram:165e4},kpr:{total:0,paid:0,monthly:0,bank:"",remaining_months:0},arisan:[]},settings:{togetherMode:!1,allowanceBudget:15e5,userName:"Erwin",spouseName:"Bunda"},categories:[]};class et{constructor(){this._listeners=[],this._state=this._load()}_load(){try{const t=localStorage.getItem($);if(t){const e=JSON.parse(t);return{...I,...e}}}catch(t){console.warn("Failed to load state",t)}return{...I}}_save(){try{localStorage.setItem($,JSON.stringify(this._state))}catch(t){console.warn("Failed to save state",t)}this._notify()}_notify(){this._listeners.forEach(t=>t(this._state))}subscribe(t){return this._listeners.push(t),()=>{this._listeners=this._listeners.filter(e=>e!==t)}}getState(){return this._state}setFamily(t){this._state.family=t,this._save()}updateSettings(t){this._state.settings={...this._state.settings,...t},this._save()}toggleTogetherMode(){return this._state.settings.togetherMode=!this._state.settings.togetherMode,this._save(),this._state.settings.togetherMode}addAccount(t){const n=this._state.accounts.reduce((s,i)=>Math.max(s,i.id||0),0)+1;return this._state.accounts.push({id:n,...t}),this._save(),n}updateAccount(t,e){const n=this._state.accounts.findIndex(s=>s.id===t);n!==-1&&(this._state.accounts[n]={...this._state.accounts[n],...e},this._save())}deleteAccount(t){this._state.accounts=this._state.accounts.filter(e=>e.id!==t),this._save()}getAccounts(){return this._state.accounts}getAccountById(t){return this._state.accounts.find(e=>e.id===t)}addTransaction(t){const e=crypto.randomUUID?crypto.randomUUID():Date.now().toString(36)+Math.random().toString(36).substr(2),n={id:e,created_at:new Date().toISOString(),...t};return this._state.transactions.unshift(n),t.type==="income"?this._updateAccountBalance(t.account_id,t.amount):t.type==="expense"?this._updateAccountBalance(t.account_id,-t.amount):t.type==="transfer"&&(this._updateAccountBalance(t.account_id,-t.amount),t.to_account_id&&this._updateAccountBalance(t.to_account_id,t.amount)),this._save(),e}_updateAccountBalance(t,e){const n=this._state.accounts.find(s=>s.id===t);n&&(n.balance=(n.balance||0)+e)}deleteTransaction(t){const e=this._state.transactions.find(n=>n.id===t);e&&(e.type==="income"?this._updateAccountBalance(e.account_id,-e.amount):e.type==="expense"?this._updateAccountBalance(e.account_id,e.amount):e.type==="transfer"&&(this._updateAccountBalance(e.account_id,e.amount),e.to_account_id&&this._updateAccountBalance(e.to_account_id,-e.amount)),this._state.transactions=this._state.transactions.filter(n=>n.id!==t),this._save())}getTransactions(t={}){let e=[...this._state.transactions];if(t.type&&(e=e.filter(n=>n.type===t.type)),t.paid_by&&(e=e.filter(n=>n.paid_by===t.paid_by)),t.for_whom&&(e=e.filter(n=>n.for_whom===t.for_whom)),t.account_id&&(e=e.filter(n=>n.account_id===t.account_id)),t.parent_category&&(e=e.filter(n=>n.parent_category===t.parent_category)),t.month&&(e=e.filter(n=>{const s=new Date(n.created_at);return s.getMonth()===t.month&&s.getFullYear()===(t.year||new Date().getFullYear())})),t.search){const n=t.search.toLowerCase();e=e.filter(s=>(s.description||"").toLowerCase().includes(n)||(s.parent_category||"").toLowerCase().includes(n)||(s.sub_category||"").toLowerCase().includes(n))}return e}getTransactionsByMonth(t,e){return this._state.transactions.filter(n=>{const s=new Date(n.created_at);return s.getFullYear()===t&&s.getMonth()===e})}updateAssets(t){this._state.assets={...this._state.assets,...t},this._save()}updateEmas(t){this._state.assets.emas={...this._state.assets.emas,...t},this._save()}updateKPR(t){this._state.assets.kpr={...this._state.assets.kpr,...t},this._save()}addArisan(t){const e=Date.now();return this._state.assets.arisan.push({id:e,...t}),this._save(),e}getAssets(){return this._state.assets}getTotalBalance(){return this._state.accounts.reduce((t,e)=>t+(e.balance||0),0)}getMonthlyExpenses(t,e){return this.getTransactionsByMonth(t,e).filter(n=>n.type==="expense").reduce((n,s)=>n+s.amount,0)}getMonthlyIncome(t,e){return this.getTransactionsByMonth(t,e).filter(n=>n.type==="income").reduce((n,s)=>n+s.amount,0)}getAllowanceSpent(){const t=new Date;return this.getTransactionsByMonth(t.getFullYear(),t.getMonth()).filter(e=>e.type==="expense"&&e.paid_by==="Suami").reduce((e,n)=>e+n.amount,0)}getDanaPusatBalance(){return this._state.accounts.filter(t=>t.owner_name==="Istri"||t.owner_name==="Bersama").reduce((t,e)=>t+(e.balance||0),0)}getNetWorth(){const t=this.getTotalBalance(),e=this._state.assets.emas,n=(e.bsi_gram+e.tring_gram)*e.price_per_gram,s=this._state.assets.kpr.paid;return t+n+s}getCategorySpending(t,e){const n=this.getTransactionsByMonth(t,e).filter(i=>i.type==="expense"),s={};return n.forEach(i=>{const o=i.parent_category||"Lainnya";s[o]=(s[o]||0)+i.amount}),Object.entries(s).map(([i,o])=>({name:i,amount:o})).sort((i,o)=>o.amount-i.amount)}reset(){this._state=JSON.parse(JSON.stringify(I)),localStorage.removeItem($),this._notify()}}const d=new et,F=4;function at(){const a=localStorage.getItem("cipta_seed_version");if(a&&parseInt(a)>=F&&d.getState().accounts.length>0)return;d.reset(),d.setFamily({id:"family-001",family_name:"Keluarga Erwin",created_at:new Date().toISOString()}),d.updateSettings({userName:"Pak Erwin",spouseName:"Bunda",allowanceBudget:15e5});const t=d.addAccount({bank_name:"BRI",owner_name:"Suami",balance:85e5,is_allowance_account:!1,css_class:"bri"}),e=d.addAccount({bank_name:"Bank Jago",owner_name:"Bersama",balance:32e5,is_allowance_account:!0,css_class:"jago"}),n=d.addAccount({bank_name:"BSI",owner_name:"Istri",balance:12e6,is_allowance_account:!1,css_class:"bsi"});d.updateEmas({bsi_gram:15.5,tring_gram:3.2,price_per_gram:165e4}),d.updateKPR({total:25e7,paid:875e5,monthly:21e5,bank:"BTN",remaining_months:96}),d.addArisan({name:"Arisan Kantor Dinas",monthly_amount:2e5,total_members:15,my_turn:8,current_round:3,is_active:!0}),d.addArisan({name:"Arisan Ibu PKK",monthly_amount:15e4,total_members:20,my_turn:14,current_round:5,is_active:!0});const s=new Date,i=s.getFullYear(),o=s.getMonth(),r=[{account_id:t,amount:72e5,type:"income",description:"Gaji Pokok + Tunjangan April",parent_category:"Gaji & Pendapatan",sub_category:"Gaji Pokok",paid_by:"Suami",for_whom:"Bersama",created_at:new Date(i,o,1,8,0).toISOString()},{account_id:t,amount:35e5,type:"income",description:"TPP Bulan April",parent_category:"Gaji & Pendapatan",sub_category:"TPP",paid_by:"Suami",for_whom:"Bersama",created_at:new Date(i,o,5,10,0).toISOString()},{account_id:t,amount:65e5,type:"transfer",description:"Transfer Dana Pusat ke BSI Istri",parent_category:"Transfer",sub_category:"Pindah Buku",paid_by:"Suami",for_whom:"Bersama",to_account_id:n,created_at:new Date(i,o,1,9,0).toISOString()},{account_id:t,amount:15e5,type:"transfer",description:"Pegangan Suami ke Jago",parent_category:"Transfer",sub_category:"Pindah Buku",paid_by:"Suami",for_whom:"Suami",to_account_id:e,created_at:new Date(i,o,1,9,30).toISOString()},{account_id:e,amount:15e4,type:"expense",description:"Bensin Raize Perjalanan LDM",parent_category:"Transportasi",sub_category:"Bensin LDM",paid_by:"Suami",for_whom:"Suami",created_at:new Date(i,o,2,7,30).toISOString()},{account_id:e,amount:35e3,type:"expense",description:"Tol Barru - Makassar",parent_category:"Transportasi",sub_category:"Tol",paid_by:"Suami",for_whom:"Suami",created_at:new Date(i,o,2,8,0).toISOString()},{account_id:e,amount:25e3,type:"expense",description:"Makan Siang Warung Pak Baso",parent_category:"Makanan & Minuman",sub_category:"Makan Harian",paid_by:"Suami",for_whom:"Suami",created_at:new Date(i,o,3,12,0).toISOString()},{account_id:e,amount:15e3,type:"expense",description:"Kopi Kenangan",parent_category:"Makanan & Minuman",sub_category:"Kopi & Minuman",paid_by:"Suami",for_whom:"Suami",created_at:new Date(i,o,3,15,0).toISOString()},{account_id:e,amount:15e4,type:"expense",description:"Bensin Raize Harian",parent_category:"Transportasi",sub_category:"Bensin Harian",paid_by:"Suami",for_whom:"Suami",created_at:new Date(i,o,5,7,0).toISOString()},{account_id:e,amount:2e5,type:"expense",description:"Arisan Kantor Bulan Ini",parent_category:"Sosial & Ibadah",sub_category:"Arisan",paid_by:"Suami",for_whom:"Suami",created_at:new Date(i,o,6,10,0).toISOString()},{account_id:n,amount:35e4,type:"expense",description:"Belanja Bulanan Indomaret",parent_category:"Makanan & Minuman",sub_category:"Groceries",paid_by:"Istri",for_whom:"Bersama",created_at:new Date(i,o,2,10,0).toISOString()},{account_id:n,amount:5e5,type:"expense",description:"SPP TK Aisyah Bulan April",parent_category:"Pendidikan Anak",sub_category:"SPP",paid_by:"Istri",for_whom:"Anak",created_at:new Date(i,o,3,9,0).toISOString()},{account_id:n,amount:18e4,type:"expense",description:"Listrik Rumah B",parent_category:"Rumah Tangga",sub_category:"Listrik",paid_by:"Istri",for_whom:"Bersama",created_at:new Date(i,o,4,11,0).toISOString()},{account_id:n,amount:21e5,type:"expense",description:"Cicilan KPR BTN April",parent_category:"Cicilan",sub_category:"KPR",paid_by:"Istri",for_whom:"Bersama",created_at:new Date(i,o,5,8,0).toISOString()},{account_id:n,amount:15e4,type:"expense",description:"Arisan Ibu PKK",parent_category:"Sosial & Ibadah",sub_category:"Arisan",paid_by:"Istri",for_whom:"Istri",created_at:new Date(i,o,6,14,0).toISOString()},{account_id:n,amount:85e3,type:"expense",description:"Vitamin & Susu Anak",parent_category:"Kesehatan",sub_category:"Vitamin",paid_by:"Istri",for_whom:"Anak",created_at:new Date(i,o,7,10,0).toISOString()},{account_id:e,amount:25e4,type:"expense",description:"Makan Keluarga di Restoran",parent_category:"Hiburan",sub_category:"Quality Time",paid_by:"Suami",for_whom:"Bersama",is_together:!0,created_at:new Date(i,o,7,19,0).toISOString()},{account_id:e,amount:45e3,type:"expense",description:"Es Krim untuk Anak",parent_category:"Makanan & Minuman",sub_category:"Cemilan",paid_by:"Suami",for_whom:"Anak",is_together:!0,created_at:new Date(i,o,7,20,0).toISOString()},{account_id:n,amount:825e3,type:"expense",description:"Beli Emas BSI 0.5 gram",parent_category:"Investasi",sub_category:"Emas BSI",paid_by:"Istri",for_whom:"Bersama",created_at:new Date(i,o,8,9,0).toISOString()}],c=d.getState();r.forEach(l=>{const u=crypto.randomUUID?crypto.randomUUID():Date.now().toString(36)+Math.random().toString(36).substr(2);c.transactions.push({id:u,...l})}),c.transactions.sort((l,u)=>new Date(u.created_at)-new Date(l.created_at)),localStorage.setItem("cipta_finansial_data",JSON.stringify(c)),localStorage.setItem("cipta_seed_version",F.toString())}const nt="modulepreload",st=function(a){return"/Admfb/"+a},q={},E=function(t,e,n){let s=Promise.resolve();if(e&&e.length>0){let o=function(l){return Promise.all(l.map(u=>Promise.resolve(u).then(m=>({status:"fulfilled",value:m}),m=>({status:"rejected",reason:m}))))};document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),c=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));s=o(e.map(l=>{if(l=st(l),l in q)return;q[l]=!0;const u=l.endsWith(".css"),m=u?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${m}`))return;const g=document.createElement("link");if(g.rel=u?"stylesheet":nt,u||(g.as="script"),g.crossOrigin="",g.href=l,c&&g.setAttribute("nonce",c),document.head.appendChild(g),u)return new Promise((h,v)=>{g.addEventListener("load",h),g.addEventListener("error",()=>v(new Error(`Unable to preload CSS for ${l}`)))})}))}function i(o){const r=new Event("vite:preloadError",{cancelable:!0});if(r.payload=o,window.dispatchEvent(r),!r.defaultPrevented)throw o}return s.then(o=>{for(const r of o||[])r.status==="rejected"&&i(r.reason);return t().catch(i)})};function it(){const a=d.getState(),t=a.settings.togetherMode;return`
  ...
  `}function ot(){const a=document.getElementById("together-toggle");a&&a.addEventListener("click",()=>{const e=d.toggleTogetherMode();a.classList.toggle("active",e);const n=a.querySelector(".together-toggle-icon"),s=a.querySelector(".together-toggle-label");n.textContent=e?"favorite":"favorite_border",s.textContent=e?"Together!":"Together",E(async()=>{const{showToast:i}=await Promise.resolve().then(()=>G);return{showToast:i}},void 0).then(({showToast:i})=>{i(e?"💕 Together Mode Aktif!":"Together Mode Nonaktif","info")})});const t=document.getElementById("profile-avatar");t&&t.addEventListener("click",()=>{const n=d.getState().settings.userName||"Erwin",s=n==="Erwin"?"Bunda":"Erwin";confirm(`Ganti sesi dari ${n} ke ${s}?`)&&(d.updateSettings({userName:s}),window.dispatchEvent(new Event("data-updated")),E(async()=>{const{showToast:i}=await Promise.resolve().then(()=>G);return{showToast:i}},void 0).then(({showToast:i})=>{i(`Berhasil login sebagai ${s}`,"success")}))})}const rt=[{path:"/",icon:"dashboard",label:"Beranda"},{path:"/transactions",icon:"receipt_long",label:"Transaksi"},{path:"/accounts",icon:"account_balance_wallet",label:"Rekening"},{path:"/assets",icon:"diamond",label:"Aset"},{path:"/insights",icon:"auto_awesome",label:"AI Insight"}];function ct(){const a=x.getCurrentPath();return`
  ...
  `}let y=null;function _t(){var s;const a=document.getElementById("scan-modal-backdrop"),t=document.getElementById("scan-modal-sheet"),e=document.getElementById("scan-upload-area"),n=document.getElementById("scan-file-input");window.addEventListener("open-scan-modal",()=>{a==null||a.classList.add("open"),t==null||t.classList.add("open"),document.getElementById("scan-preview").style.display="none",document.getElementById("scan-result").style.display="none",document.getElementById("scan-loading").style.display="none",document.getElementById("scan-parsed-data").style.display="none",document.getElementById("scan-save-btn").style.display="none",y=null}),a==null||a.addEventListener("click",()=>{a==null||a.classList.remove("open"),t==null||t.classList.remove("open")}),e==null||e.addEventListener("click",()=>n==null?void 0:n.click()),n==null||n.addEventListener("change",async i=>{var c;const o=(c=i.target.files)==null?void 0:c[0];if(!o)return;const r=new FileReader;r.onload=async l=>{const u=l.target.result;document.getElementById("scan-preview-img").src=u,document.getElementById("scan-preview").style.display="block",document.getElementById("scan-loading").style.display="block",document.getElementById("scan-result").style.display="none";try{await kt(u.split(",")[1],o.type)}catch(m){console.error("AI Error:",m),document.getElementById("scan-loading").style.display="none",document.getElementById("scan-result").style.display="block",document.getElementById("scan-ai-text").textContent="❌ Gagal menganalisis gambar. Silakan coba lagi atau input manual."}},r.readAsDataURL(o)}),(s=document.getElementById("scan-save-btn"))==null||s.addEventListener("click",()=>{var r;if(!y)return;const o=(r=d.getAccounts()[0])==null?void 0:r.id;d.addTransaction({account_id:o,amount:y.amount||0,type:y.type||"expense",description:y.description||"Transaksi dari Scan",parent_category:y.category||"Lainnya",sub_category:y.sub_category||"",paid_by:d.getState().settings.togetherMode?"Istri":"Suami",for_whom:d.getState().settings.togetherMode?"Bersama":"Suami",is_together:d.getState().settings.togetherMode,created_at:y.date?new Date(y.date).toISOString():new Date().toISOString()}),_("✅ Transaksi dari scan berhasil disimpan!"),a==null||a.classList.remove("open"),t==null||t.classList.remove("open"),window.dispatchEvent(new CustomEvent("data-updated"))})}async function kt(a,t){var r,c,l,u,m,g;const n=`Analisis gambar struk/nota/slip gaji berikut. Ekstrak informasi keuangan dan kembalikan dalam format JSON SAJA (tanpa markdown):
{
  ...
  "category": salah satu dari [${B.map(h=>h.name).join(", ")}],
...
Jika ini slip gaji PNS, ekstrak: Gaji Pokok, Tunjangan, Potongan, dan Total Take Home Pay. Set type ke "income" dan amount ke take home pay.`,o=((m=(u=(l=(c=(r=(await(await fetch(ft,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:n},{inline_data:{mime_type:t||"image/jpeg",data:a}}]}],generationConfig:{temperature:.1,maxOutputTokens:1024}})})).json()).candidates)==null?void 0:r[0])==null?void 0:c.content)==null?void 0:l.parts)==null?void 0:u[0])==null?void 0:m.text)||"";document.getElementById("scan-loading").style.display="none",document.getElementById("scan-result").style.display="block";try{const h=o.match(/\{[\s\S]*\}/);if(h){y=JSON.parse(h[0]),document.getElementById("scan-ai-text").innerHTML=`
      ...
      `,document.getElementById("scan-save-btn").style.display="flex"}else document.getElementById("scan-ai-text").textContent=o||"Tidak dapat mengekstrak data dari gambar ini."}catch{document.getElementById("scan-ai-text").textContent=o||"Gagal memproses respons AI."}}function xt(){const a=d.getState(),t=new Date,e=a.settings.allowanceBudget||15e5,n=d.getAllowanceSpent(),s=Math.max(0,e-n),i=k(n,e),o=d.getDanaPusatBalance(),r=d.getMonthlyExpenses(t.getFullYear(),t.getMonth()),c=d.getMonthlyIncome(t.getFullYear(),t.getMonth());return`
  ...
  `}function wt(a){const t=a.toLowerCase();return t.includes("bri")?"bri":t.includes("jago")?"jago":t.includes("bsi")?"bsi":"default"}function St(a){const t=a.toLowerCase();return t.includes("bri")?"BRI":t.includes("jago")?"JGO":t.includes("bsi")?"BSI":a.slice(0,3).toUpperCase()}function Et(){return`
    ...
    <div class="bank-slider" id="bank-slider">
      ${d.getAccounts().map(t=>{const e=wt(t.bank_name);return`
  ...
  `}function Bt(){const a=document.getElementById("btn-add-account");a&&a.addEventListener("click",()=>{window.dispatchEvent(new CustomEvent("open-account-modal"))});const t=document.getElementById("btn-manage-accounts");t&&t.addEventListener("click",()=>{E(async()=>{const{default:e}=await Promise.resolve().then(()=>tt);return{default:e}},void 0).then(({default:e})=>e.navigate("/accounts"))})}function $t(){const a=d.getState(),t=new Date,e=d.getTransactionsByMonth(t.getFullYear(),t.getMonth()),n=a.assets.kpr,s=k(n.paid,n.total),i=6e5,o=e.filter(m=>m.type==="expense"&&m.parent_category==="Transportasi").reduce((m,g)=>m+g.amount,0),r=k(o,i),c=8e5,l=e.filter(m=>m.type==="expense"&&m.for_whom==="Anak").reduce((m,g)=>m+g.amount,0),u=k(l,c);return`
    <div class="section-header">
  ...
  `}function P(a=10,t={}){let e=d.getTransactions(t);const n=e.length;if(a&&(e=e.slice(0,a)),e.length===0)return`
      <div class="empty-state">
        <span class="material-icons-round">receipt_long</span>
        <h3>Belum Ada Transaksi</h3>
        <p>Mulai catat pengeluaran dan pemasukan Anda</p>
      </div>
    `;const s=W(e);let i="";return Object.entries(s).forEach(([o,r])=>{i+=`<div class="transaction-group-header">${o}</div>`,r.forEach(c=>{const l=pt(c.parent_category),u=c.type,m=c.type==="expense"?"-":c.type==="income"?"+":"↔",g=d.getAccountById(c.account_id),h=g?g.bank_name:"";i+=`
    ...
    `),i}function M(){document.querySelectorAll(".transaction-item").forEach(a=>{a.addEventListener("click",()=>{const t=a.dataset.txId;window.dispatchEvent(new CustomEvent("view-transaction",{detail:{id:t}}))})})}function It(){const a=d.getState();return`
  ...
  `}function At(){Bt(),M()}function Tt(){const a=new Date().getHours();return a<11?"Selamat Pagi":a<15?"Selamat Siang":a<18?"Selamat Sore":"Selamat Malam"}let b={};function Pt(){const a=new Date,t=d.getMonthlyExpenses(a.getFullYear(),a.getMonth()),e=d.getMonthlyIncome(a.getFullYear(),a.getMonth()),n=e-t;return`
  ...
  `}function Mt(){M(),document.querySelectorAll("#tx-filters .chip").forEach(t=>{t.addEventListener("click",()=>{document.querySelectorAll("#tx-filters .chip").forEach(n=>n.classList.remove("selected")),t.classList.add("selected");const e=t.dataset.filter;e==="all"?delete b.type:b.type=e,A()})}),document.querySelectorAll("#tx-paid-filter .chip").forEach(t=>{t.addEventListener("click",()=>{document.querySelectorAll("#tx-paid-filter .chip").forEach(n=>n.classList.remove("selected")),t.classList.add("selected");const e=t.dataset.paid;e==="all"?delete b.paid_by:b.paid_by=e,A()})});const a=document.getElementById("tx-search");a&&a.addEventListener("input",V(t=>{const e=t.target.value.trim();e?b.search=e:delete b.search,A()},300)),Lt()}function A(){const a=document.getElementById("filtered-transactions");a&&(a.innerHTML=P(null,b),M())}async function Lt(){const a=document.getElementById("category-chart");if(a)try{const t=await E(()=>import("./auto-eE5P6S0m.js"),[]),e=t.default||t.Chart,n=new Date,s=d.getCategorySpending(n.getFullYear(),n.getMonth());if(s.length===0){a.parentElement.innerHTML=`
      ...
      `;return}const i=["#30609d","#9a6a1a","#1b6d2f","#ba1a1a","#7b1fa2","#00695c","#e65100","#283593","#4e342e","#546e7a","#ad1457","#00838f"];new e(a,{type:"doughnut",data:{labels:s.map(o=>o.name),datasets:[{data:s.map(o=>o.amount),backgroundColor:i.slice(0,s.length),borderWidth:2,borderColor:"#fff",hoverOffset:6}]},options:{responsive:!0,maintainAspectRatio:!1,cutout:"65%",plugins:{legend:{position:"bottom",labels:{padding:12,usePointStyle:!0,pointStyleWidth:8,font:{family:"'Plus Jakarta Sans'",size:11,weight:"600"}}},tooltip:{callbacks:{label:o=>{const r=o.dataset.data.reduce((l,u)=>l+u,0),c=Math.round(o.parsed/r*100);return` ${o.label}: ${p(o.parsed)} (${c}%)`}},titleFont:{family:"'Plus Jakarta Sans'"},bodyFont:{family:"'Plus Jakarta Sans'"}}}}})}catch(t){console.warn("Chart.js not loaded:",t)}}function Dt(){const a=d.getAccounts(),t=d.getTotalBalance();return`
  ...
  `}function jt(){var e;const a=document.getElementById("btn-add-new-account"),t=document.getElementById("acc-modal-backdrop");document.getElementById("acc-modal-sheet"),a==null||a.addEventListener("click",()=>H()),window.addEventListener("open-account-modal",()=>H()),t==null||t.addEventListener("click",()=>U()),document.querySelectorAll("#acc-owner-chips .chip").forEach(n=>{n.addEventListener("click",()=>{document.querySelectorAll("#acc-owner-chips .chip").forEach(s=>s.classList.remove("selected")),n.classList.add("selected")})}),(e=document.getElementById("acc-form"))==null||e.addEventListener("submit",n=>{var c,l,u,m,g;n.preventDefault();const s=(l=(c=document.getElementById("acc-bank-name"))==null?void 0:c.value)==null?void 0:l.trim(),i=((u=document.querySelector("#acc-owner-chips .chip.selected"))==null?void 0:u.dataset.value)||"Suami",o=parseFloat(((m=document.getElementById("acc-balance"))==null?void 0:m.value)||0),r=((g=document.getElementById("acc-is-allowance"))==null?void 0:g.checked)||!1;if(!s){_("Masukkan nama bank","error");return}d.addAccount({bank_name:s,owner_name:i,balance:o,is_allowance_account:r,css_class:Q(s).replace("bank-card-icon ","")}),_("✅ Rekening berhasil ditambahkan!"),U(),window.dispatchEvent(new CustomEvent("data-updated"))}),document.querySelectorAll(".btn-delete-account").forEach(n=>{n.addEventListener("click",s=>{s.stopPropagation();const i=parseInt(n.dataset.accId);confirm("Hapus rekening ini?")&&(d.deleteAccount(i),_("Rekening dihapus"),window.dispatchEvent(new CustomEvent("data-updated")))})})}function H(){var a,t,e;(a=document.getElementById("acc-form"))==null||a.reset(),(t=document.getElementById("acc-modal-backdrop"))==null||t.classList.add("open"),(e=document.getElementById("acc-modal-sheet"))==null||e.classList.add("open")}function U(){var a,t;(a=document.getElementById("acc-modal-backdrop"))==null||a.classList.remove("open"),(t=document.getElementById("acc-modal-sheet"))==null||t.classList.remove("open")}function Q(a){const t=a.toLowerCase();return t.includes("bri")?"bri":t.includes("jago")?"jago":t.includes("bsi")?"bsi":"default"}function Rt(a){const t=a.toLowerCase();return t.includes("bri")?"BRI":t.includes("jago")?"JGO":t.includes("bsi")?"BSI":a.slice(0,3).toUpperCase()}function Ot(){const a=d.getAssets(),t=d.getNetWorth(),e=d.getTotalBalance(),n=a.emas,s=n.bsi_gram+n.tring_gram,i=s*n.price_per_gram,o=a.kpr,r=k(o.paid,o.total),c=o.total-o.paid,l=a.arisan||[];return`
  ...
  `}function Kt(){}function Nt(){d.getState();const a=new Date,t=Ft();return`
      ...
      ${t.map((s,i)=>{const o=Math.round(s.amount/e*100),r=["#30609d","#9a6a1a","#1b6d2f","#ba1a1a","#7b1fa2","#00695c","#e65100","#283593"],c=r[i%r.length];return`
          <div style="margin-bottom: 12px;">
            ...
            <div style="width: 100%; height: 6px; background: var(--surface-container); border-radius: 3px; overflow: hidden;">
              <div style="width: ${o}%; height: 100%; background: ${c}; border-radius: 3px; transition: width 0.8s ease;"></div>
            </div>
          </div>
        `}).join("")}
    </div>
  `}function Gt(){}const Jt=document.getElementById("app"),T={"/":{render:It,init:At},"/transactions":{render:Pt,init:Mt},"/accounts":{render:Dt,init:jt},"/assets":{render:Ot,init:Kt},"/insights":{render:Nt,init:Gt}};function Z(a){const t=T[a]||T["/"];Jt.innerHTML=`
    ${it()}
    ${ct()}
    ${t.render()}
    ${lt()}
