...
[
    ...
    `;const w=c.getAccounts(),b=document.getElementById("scan-parsed-data");b.style.display="block",b.innerHTML=$.map(f=>{let S="Tunai (Otomatis)";if(f.account_guess){const A=f.account_guess.toLowerCase(),x=w.find(I=>I.bank_name.toLowerCase().includes(A));x&&(S=x.bank_name)}return`
        <div style="background: var(--surface-container); border-radius: var(--radius-md); padding: 12px; margin-top: 12px; border: 1px solid var(--outline-variant);">
          ...
          <div style="font-size: 13px; color: var(--on-surface-variant);">
            ...
            <p>💳 Menggunakan: <strong>${S} ${f.to_account_guess?" ➡️ "+f.to_account_guess.toUpperCase():""}</strong></p>
          </div>
        </div>
      `}).join(""),document.getElementById("scan-save-btn").style.display="flex"}catch(h){throw console.error(h,l),new Error("Parsing Error: "+h.message)}}function xe(){const a=c.getState(),e=new Date,t=a.settings.allowanceBudget||15e5,n=c.getAllowanceSpent(),s=Math.max(0,t-n),i=T(n,t),o=c.getDanaPusatBalance(),r=c.getMonthlyExpenses(e.getFullYear(),e.getMonth()),d=c.getMonthlyIncome(e.getFullYear(),e.getMonth());return`
  ...
  `}function Be(){const a=document.getElementById("btn-add-account");a&&a.addEventListener("click",()=>{window.dispatchEvent(new CustomEvent("open-account-modal"))});const e=document.getElementById("btn-manage-accounts");e&&e.addEventListener("click",()=>{D(async()=>{const{default:t}=await Promise.resolve().then(()=>se);return{default:t}},void 0).then(({default:t})=>t.navigate("/accounts"))})}function $e(){const a=c.getState(),e=new Date,t=c.getTransactionsByMonth(e.getFullYear(),e.getMonth()),n=a.assets.kpr,s=T(n.paid,n.total),i=6e5,o=t.filter(m=>m.type==="expense"&&m.parent_category==="Transportasi").reduce((m,g)=>m+g.amount,0),r=T(o,i),d=8e5,u=t.filter(m=>m.type==="expense"&&m.for_whom==="Anak").reduce((m,g)=>m+g.amount,0),l=T(u,d);return`
    <div class="section-header">
  ...
  `}function R(a=10,e={}){let t=c.getTransactions(e);const n=t.length;if(a&&(t=t.slice(0,a)),t.length===0)return`
    ...
    `),i}function N(){document.querySelectorAll(".transaction-item").forEach(a=>{a.addEventListener("click",()=>{const e=a.dataset.txId;window.dispatchEvent(new CustomEvent("view-transaction",{detail:{id:e}}))})})}function Ae(){const a=c.getState();return`
  ...
  `}function Te(){Be(),N()}function Pe(){const a=new Date().getHours();return a<11?"Selamat Pagi":a<15?"Selamat Siang":a<18?"Selamat Sore":"Selamat Malam"}let B={};function Me(){const a=new Date,e=c.getMonthlyExpenses(a.getFullYear(),a.getMonth()),t=c.getMonthlyIncome(a.getFullYear(),a.getMonth()),n=t-e;return`
  ...
  `}function Le(){N(),document.querySelectorAll("#tx-filters .chip").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll("#tx-filters .chip").forEach(n=>n.classList.remove("selected")),e.classList.add("selected");const t=e.dataset.filter;t==="all"?delete B.type:B.type=t,K()})}),document.querySelectorAll("#tx-paid-filter .chip").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll("#tx-paid-filter .chip").forEach(n=>n.classList.remove("selected")),e.classList.add("selected");const t=e.dataset.paid;t==="all"?delete B.paid_by:B.paid_by=t,K()})});const a=document.getElementById("tx-search");a&&a.addEventListener("input",ee(e=>{const t=e.target.value.trim();t?B.search=t:delete B.search,K()},300)),De()}function K(){const a=document.getElementById("filtered-transactions");a&&(a.innerHTML=R(null,B),N())}async function De(){const a=document.getElementById("category-chart");if(a)try{const e=await D(()=>import("./auto-eE5P6S0m.js"),[]),t=e.default||e.Chart,n=new Date,s=c.getCategorySpending(n.getFullYear(),n.getMonth());if(s.length===0){a.parentElement.innerHTML=`
  ...
  `}function Ne(){}function Fe(){c.getState();const a=new Date,e=Je();return`
  ...
  `}function He(){}const qe=document.getElementById("app"),O={"/":{render:Ae,init:Te},"/transactions":{render:Me,init:Le},"/accounts":{render:ze,init:Ke},"/assets":{render:Re,init:Ne},"/insights":{render:Fe,init:He}};function ae(a){const e=O[a]||O["/"];qe.innerHTML=`
    ${de()}
    ${pe()}
    ${e.render()}
    ${ge()}
    ${he()}
    ${ke()}
  `,le(),ve(),be(),we(),e.init(),me(a),window.scrollTo({top:0,behavior:"instant"})}Object.keys(O).forEach(a=>{M.addRoute(a,ae)});window.addEventListener("data-updated",()=>{const a=M.getCurrentPath();ae(a)});function Ue(){oe(),M.start()}document.addEventListener("DOMContentLoaded",Ue);
